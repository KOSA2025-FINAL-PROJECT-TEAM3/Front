...
**질문 내용**  
정의서(`md/FRONTEND_COMPONENTS_SPECIFICATION.md`, `md/SRC_STRUCTURE.md`)에는 `AuthContext.jsx` 구조가 명시돼 있지만, 현재 구현은 Zustand 스토어(`src/features/auth/store/authStore.js`)만 사용하고 있습니다. 전역 상태는 반드시 Zustand를 써야 한다는 규칙을 지키면서도 문서와 구현이 어긋난 상황을 어떻게 정리해야 할까요?
...
**시도한 방법**  
- Zustand store를 단일 소스로 재정비(`src/features/auth/store/authStore.js`, `src/features/auth/hooks/useAuth.js`)  
...
- 관련 코드: `src/features/auth/store/authStore.js`, `src/features/auth/hooks/useAuth.js`, `src/App.jsx`  
