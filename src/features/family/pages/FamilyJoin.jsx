...
export default function FamilyJoin() {
  ...
  useEffect(() => {
    ...
    const shortFromUrl = searchParams.get('code')
    if (shortFromUrl) {
      setCode(shortFromUrl.toUpperCase())
    }
  }, [searchParams])
    ...
    try {
      ...
      switch (errorCode) {
        ...
        case 'IDENTITY_MISMATCH':
          ...
          break
        case 'INVITE_EXPIRED':
          setError('만료되었거나 사용된 초대입니다.')
          break
        case 'ROLE_CONFLICT':
          setError(message || '현재 역할과 달라 수락할 수 없습니다.')
          break
        case 'ALREADY_MEMBER':
          setError('이미 해당 가족의 구성원입니다.')
          navigate(ROUTE_PATHS.family)
          break
        case 'RATE_LIMIT':
    ...
    <div className="family-join-page">
      <div className="family-join-container">
        ...
        <form onSubmit={handleSubmit}>
          <div className="code-input-group">
            ...
            <input
              ...
              autoFocus
            />
            <small className="hint">이메일이나 메시지로 받은 코드를 입력하세요.</small>
          </div>
        ...
        <div className="signup-link">
          <p>아직 회원이 아니신가요?</p>
          <a href={ROUTE_PATHS.signup}>회원가입</a>
        </div>
      </div>
    </div>
  )
